...
<div class="fieldcontain ${hasErrors(bean: topicInstance, field: 'createdBy', 'error')} required">
    ...
    <g:select id="createdBy" name="createdBy.id" from="${com.ttnd.linksharing.User.list()}" optionKey="id" required=""
...
<div class="fieldcontain ${hasErrors(bean: topicInstance, field: 'visibility', 'error')} required">
    ...
    <g:select name="visibility" from="${com.enums.Visibility?.values()}" keys="${com.enums.Visibility.values()*.name()}"
              required="" value="${topicInstance?.visibility?.name()}"/>
